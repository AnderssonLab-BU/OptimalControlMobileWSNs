function [T_list, R_list, Phi_list,S_list] = Data_collecting(Env_info,Target_info,Theta,W,P)
...
for i = 1:n_visiting_time
    ...
    while Singular_Arc_Distance > Tolerance
        ...
        R_list=[R_list R'];
        T_list=[T_list T];
        Phi_list=[Phi_list data_rate'];
        s_agent_target=[s_agent;X_target'];
        S_list=[S_list s_agent_target];
        if s_agent > 10
            break
        end
        if s_agent < 0
...
while R_Data_Satisfied(R,r) == 0
    [X_target,target_velocity]=Target_State_Update(X_target,target_velocity,time_step,target_velocity_intial,T,period_list);
    u_speed = 0;
    data_rate = repelem(0.1,n_target); %To avoid the zero data rate and hold on
    R = R + time_step*data_rate;
    s_agent =s_agent + time_step*u_speed;
    T = T + time_step;
    R_list=[R_list R'];
